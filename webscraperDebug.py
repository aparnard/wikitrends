...
def ScrapeTwoMonths(keyword, year, startmonth):
    ...
    # fileout = "{0}/{1}_{2}_{3}".format(path, keyword, year, startmonth)    
    # print URL
    # try:
    #     response = urllib2.urlopen(URL, timeout = 10)
    # except:
    #     print "Cannot get data for {0} {1} {2}".format(keyword, year, startmonth)
    #     return
    # if response.getcode() is not 200:
    #     print "Cannot get data for {0} {1} {2}".format(keyword, year, startmonth)
    #     return
    
    # with open(fileout,'wb') as write_to:
    #     write_to.f.write(response.read())
    webbrowser.open(URL)    
...
def ScrapeRangeWeekly(keyword, startmonth, startyear, endmonth, endyear):
    ...
    # print "mv {0} {1}".format(oldname, newname)
    os.rename(oldname,newname)
...
def CreateDailyFrame():
    ...
    for i in range(0,len(files)):
        if files[i].lower().endswith('.csv'):
            ...
            f=open(oldname,'rU')
            temp_file = csv.reader(x.replace('\0', '') for x in f)
            # print "Opening {0}".format(newname)
...
def CreateWeeklyFrame():
    ...
    f=open(oldname,'rU')
    temp_file = csv.reader(x.replace('\0', '') for x in f)
    # print "Translating from {0} to {1}".format(oldname, newname)
    with open(newname,'wb') as write_to:
        write_data = csv.writer(write_to, delimiter=',')
        for row in temp_file:
            if len(row) == 2:
                if re.search(date_pattern,row[0]) is not None:
                    write_data.writerows([row])
                elif re.search(date_pattern2,row[0]) is not None:
                    x = "{0}-{0}".format(row[0])
                    y = [x, row[1]]
...
def ReadWeeklyFrame():
    newname = path+'/'+scrapings_dir+'/'+'weekly.csv'
    return pd.read_csv(newname,index_col=None,header=None)
...
def StitchFrames():
    ...
    # daily_frame = ReadDailyFrame()
    # interim_weekly_frame = ReadWeeklyFrame()
    ...
    for i in range(0,len(interim_weekly_frame['Date_Range'])):
        ...
        matches = re.findall(date_pattern,interim_weekly_frame['Date_Range'][i])
        if len(matches) >= 2:
            #print matches
            startdates.append(matches[0])
            enddates.append(matches[1])
        else:   
            print "NOT appending {0}".format(interim_weekly_frame['Date_Range'][i])
    ...
    for i in range(0,len(bins)):
        for j in range(0,len(bins[i])):
            try:
                keyx = str(bins[i][j].date())
                print keyx
                if not keyx in daily_frame['Daily_Volume'].index:
                    print "Key {0} not found in daily_frame".format(keyx)
                    continue
                x = long(daily_frame['Daily_Volume'][keyx])
                
                keyy = str(bins[i][0].date())
                if not keyy in weekly_frame['Weekly_Volume'].index:
                    print "Key {0} not found in weekly_frame".format(keyy)
                    continue
                y = long(weekly_frame['Weekly_Volume'][keyy])
                    
                if not keyy in daily_frame['Daily_Volume'].index:
                    print "Key {0} not found in daily_frame".format(keyy)
                    continue
                z = long(daily_frame['Daily_Volume'][keyy])

                final_data[bins[i][j]] = y*x/z
            except KeyError:                
                continue
...
if __name__ == '__main__':
    ...
    endmonth =  "4" #'4'
    endyear =   "2016" # '2016'
    keywords = ['Bernie Sanders','Hillary Clinton','Ted Cruz','John Kasich','String theory','Megan Fox','Shah Rukh Khan','Sundar Pichai','Deep Learning','MapReduce','Independence Day (United States)','Sam Anderson (Tamil actor)','Jack Ma','Ryan Gosling','Donald Trump']

    path = "C:/Users/Elizabeth/Downloads"

    for topic in keywords:
        scrapings_dir = 'gt_{0}'.format(topic)
        if not os.path.exists(path+"/"+scrapings_dir):
            os.makedirs(path+"/"+scrapings_dir)

        ScrapeRange(topic, int(startmonth), int(startyear), int(endmonth), int(endyear))
        ScrapeRangeWeekly(topic, int(startmonth), int(startyear), int(endmonth), int(endyear))
